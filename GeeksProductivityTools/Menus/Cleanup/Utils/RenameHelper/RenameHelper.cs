...
namespace Geeks.GeeksProductivityTools.Menus.Cleanup.Renaming
{
    ...
    {
        private static OUTPUT RenameSymbol(Document document, SyntaxNode root, SyntaxNode startNode, ParameterSyntax declarationNode, string newName)
        {
            ...
            var newSolution = RenameSymbol(document, annotatedRoot, identifierToken, methodAnnotation, newName);

            return GetNewStartNode(newSolution, document, methodAnnotation, startNode);
        }
        private static OUTPUT RenameSymbol(Document document, SyntaxNode root, SyntaxNode startNode, VariableDeclaratorSyntax declarationNode, string newName)
        {
            var identifierToken = declarationNode.Identifier;
        ...
        private static OUTPUT GetNewStartNode(Solution newSolution, Document document, SyntaxAnnotation methodAnnotation, SyntaxNode startNode)
        {
            var newDocument =
        ...
        public static OUTPUT RenameSymbol(Document document, SyntaxNode root, SyntaxNode startNode, SyntaxToken identifierToken, string newName)
        {
            return RenameSymbol(document, root, startNode, identifierToken.Parent, newName);
        }
        public static OUTPUT RenameSymbol(Document document, SyntaxNode root, SyntaxNode startNode, SyntaxNode declarationNode, string newName)
        {
        ...
        ////public static SyntaxNode RenameSymbolAsync(Document document, SyntaxNode root, SyntaxNode startNode, VariableDeclaratorSyntax declarationNode, string newName)
        ////{
        ////    var declarationToken2 = declarationNode.Identifier;

        ////    var myAnnotation = new SyntaxAnnotation("my");
        ////    var changeDic = new Dictionary<SyntaxNode, SyntaxNode>();
        ////    changeDic.Add(startNode, startNode.WithAdditionalAnnotations(myAnnotation));
        ////    changeDic.Add(declarationNode, declarationNode.WithIdentifier(declarationToken2.WithAdditionalAnnotations(RenameAnnotation.Create())));
        ...
        ////    annotatedRoot = annotatedDocument.GetSyntaxRootAsync().Result;
        ////    var annotatedToken = annotatedRoot.FindToken(declarationToken2.SpanStart);

        ////    var semanticModel = annotatedDocument.GetSemanticModelAsync().Result;
        ////    var symbol = semanticModel.GetDeclaredSymbol(annotatedToken.Parent);


        ////    var newSolution = Renamer.RenameSymbolAsync(annotatedSolution, symbol, newName, annotatedSolution.Workspace.Options).Result;

        ////    var newRoot =
        ////        newSolution.Projects.FirstOrDefault(x => x.Name == document.Project.Name)
        ////        .Documents.FirstOrDefault(x => x.Name == document.Name).GetSyntaxRootAsync().Result;

        ////    return newRoot.GetAnnotatedNodes(myAnnotation).FirstOrDefault();
        ////}
        private static Solution RenameSymbol(Document document, SyntaxNode annotatedRoot, SyntaxToken identifierNode, SyntaxAnnotation methodAnnotation, string newName)
        {
            var annotatedSolution = document.Project.Solution.WithDocumentSyntaxRoot(document.Id, annotatedRoot);
            var annotatedDocument = annotatedSolution.GetDocument(document.Id);
            annotatedRoot = annotatedDocument.GetSyntaxRootAsync().Result;
